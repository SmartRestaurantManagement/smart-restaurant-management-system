'use client'

import { useEffect, useState, useMemo } from 'react'
import { createClient } from '@/lib/supabase/client'
import { getCallerRestaurantId } from '@/lib/api/restaurant'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { EmptyState } from '@/components/staff/empty-state'
import { Users, RefreshCw } from 'lucide-react'

interface CustomerProfile {
  id: string
  full_name: string | null
  created_at: string
}

export default function CustomersDashboardPage() {
  const supabase = useMemo(() => createClient(), [])
  const [customers, setCustomers] = useState<CustomerProfile[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    let active = true
    let channel: ReturnType<typeof supabase.channel> | null = null

    async function loadCustomers(rid: string) {
      const { data, error: profileError } = await supabase
        .from('profiles')
        .select('*')
        .eq('restaurant_id', rid)
        .eq('role', 'customer')
        .order('full_name')

      if (!active) return

      if (profileError) {
        setError(profileError.message)
      } else {
        setCustomers((data as unknown as CustomerProfile[]) || [])
      }
      setLoading(false)
    }

    (async () => {
      setLoading(true)
      let rid = await getCallerRestaurantId(supabase)
      if (!rid) {
        const { data } = await supabase.from('restaurants').select('id').limit(1).single()
        rid = data?.id || null
      }

      if (!active) return

      if (!rid) {
        setError("Could not resolve restaurant ID.")
        setLoading(false)
        return
      }

      await loadCustomers(rid)

      const chan = supabase
        .channel('customers-realtime')
        .on(
          'postgres_changes',
          {
            event: '*',
            schema: 'public',
            table: 'profiles',
            filter: `restaurant_id=eq.${rid}`
          },
          () => {
            void loadCustomers(rid)
          }
        )

      channel = chan
      if (active) {
        chan.subscribe()
      }
    })()

    return () => {
      active = false
      if (channel) {
        supabase.removeChannel(channel)
      }
    }
  }, [supabase])

  return (
    <div className="flex flex-col gap-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-neutral-800">Customer Catalog</h1>
        <p className="text-xs text-neutral-500 mt-1">
          Review details of all registered customers.
        </p>
      </div>

      {error && <p className="text-sm text-destructive bg-red-50 border border-red-100 rounded-xl p-3">{error}</p>}

      {loading ? (
        <p className="text-sm text-muted-foreground py-8 text-center flex items-center justify-center gap-2">
          <RefreshCw className="h-4 w-4 animate-spin text-amber-600" />
          <span>Loading customer list...</span>
        </p>
      ) : customers.length === 0 ? (
        <EmptyState message="No customer accounts registered yet." />
      ) : (
        <Card className="border border-neutral-200 shadow-sm rounded-2xl overflow-hidden bg-white">
          <CardHeader className="bg-neutral-50/50 p-4 border-b border-neutral-100 flex flex-row items-center gap-2">
            <Users className="h-4 w-4 text-neutral-500" />
            <CardTitle className="text-sm font-bold text-neutral-800">
              Active Customer Profiles ({customers.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-neutral-50/50 text-neutral-400 font-semibold border-b border-neutral-100">
                    <th className="p-4">Customer Name</th>
                    <th className="p-4">Customer ID</th>
                    <th className="p-4">Account Type</th>
                    <th className="p-4">Registered Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-100">
                  {customers.map((c) => (
                    <tr key={c.id} className="hover:bg-neutral-50/30">
                      <td className="p-4 font-bold text-neutral-800 flex items-center gap-2">
                        <div className="bg-neutral-100 h-7 w-7 rounded-full flex items-center justify-center text-neutral-600 font-bold">
                          {c.full_name?.charAt(0).toUpperCase() || 'C'}
                        </div>
                        <span>{c.full_name || 'Guest User'}</span>
                      </td>
                      <td className="p-4 text-neutral-400 font-mono">{c.id}</td>
                      <td className="p-4">
                        <Badge className="bg-neutral-100 text-neutral-700 font-medium border-0">
                          Dine-In Customer
                        </Badge>
                      </td>
                      <td className="p-4 text-neutral-500">
                        {new Date(c.created_at).toLocaleDateString(undefined, {
                          year: 'numeric',
                          month: 'short',
                          day: 'numeric'
                        })}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          </CardContent>
        </Card>
      )}
    </div>
  )
}
